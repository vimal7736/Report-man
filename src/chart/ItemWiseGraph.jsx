...
const ItemWiseGraph = ({ data }) => {
  const maxQuantity = Math.max(
    ...data.map((item) => parseFloat(item.quantity))
  );

  return (
    <div className="Frame305 item-wise-graph h-[300px]  flex-col justify-start items-start rounded shadow inline-flex ">
      <div className="Frame235 self-stretch h-8 justify-between items-center inline-flex grow">
        ...
        <div className="ItemWiseGraph text-neutral-900 text-base font-semibold font-['Poppins']">Item wise graph</div>
        </div>
          ...
          {data.map((item, index) => (
            ...
            >
              <div className="Frame225 grow shrink basis-0 h-[2rem] justify-start items-start gap-2 flex">
                ...
                >
                  <div
                    className={`Rectangle35 self-stretch h-2.5  bg-${item.color}`}
                    style={{
                      width: `${
                        (parseFloat(item.quantity) / maxQuantity) * 100
                      }%`,
                    }}
                  />
                </div>
